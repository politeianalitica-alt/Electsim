"""Perfiles runtime support under Alembic.

Revision ID: 0009_perfiles_runtime_support
...
revision: str = "0009_perfiles_runtime_support"
...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_perfil_problemas_rank ON perfil_problemas (cluster_id, ranking)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_perfil_voto_cluster ON perfil_voto (cluster_id)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_perfil_ccaa_cluster ON perfil_ccaa (cluster_id)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_perfil_ejes_cluster ON perfil_ejes (cluster_id)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_pp_usuario ON perfiles_personalizados (usuario, actualizado_en DESC)")


def downgrade() -> None:
    # Revision de consolidacion legacy: no se automatiza downgrade para evitar
    # borrar tablas de perfiles con datos ya recalculados por el usuario.
    pass
