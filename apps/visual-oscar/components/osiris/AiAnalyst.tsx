...
export default function AiAnalyst({ data }: AiAnalystProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/osiris/ai/analyze', {
        method: 'POST',
        headers: getHeaders(),
      ...
      const message = err instanceof Error ? err.message : 'El análisis ha fallado';
  ...
  const handleBriefing = useCallback(async () => {
    ...
    try {
      const context = buildContext(data);
      const res = await fetch('/api/osiris/ai/briefing', {
        method: 'POST',
        headers: getHeaders(),
      ...
      const message = err instanceof Error ? err.message : 'La generación del informe ha fallado';
      const errorMsg: ChatMessage = {
        id: generateId(),
        role: 'analyst',
  ...
  const triggerButton = (
    <motion.button
      ...
      aria-label="Abrir analista de inteligencia con IA"
    >
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="hud-text text-[11px] text-[var(--text-heading)]">ANALISTA POLITEIA</span>
                    <span className="text-[7px] font-mono tracking-[0.2em] text-[var(--text-muted)]">
                      GEMINI 2.0 FLASH • EN LÍNEA
                    </span>
                  </div>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        {apiKeyInput.trim() && (
                          <>
                            ...
                            >
                              {keySaved ? <Check className="w-3 h-3" /> : 'GUARDAR'}
                            </button>
                            <button
                              onClick={clearApiKey}
                      ...
                      <p className="text-[8px] font-mono text-[var(--text-muted)] leading-relaxed">
                        Tu clave se guarda localmente y solo se envía al servidor de POLITEIA. Consigue una clave gratis en{' '}
                        <a
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center px-6 gap-5">
                    ...
                    <div className="space-y-2">
                      <h3 className="hud-text text-[12px] text-[var(--text-heading)]">
                        ANALISTA DE INTELIGENCIA LISTO
                      </h3>
                      <p className="text-[10px] font-mono text-[var(--text-muted)] leading-relaxed max-w-[280px]">
                        Correlaciono datos sísmicos, OSINT, de amenazas y ciberseguridad en tiempo real para ofrecer evaluaciones de inteligencia accionables.
                      </p>
                    </div>
                    ...
                    <div className="w-full space-y-1.5">
                      ...
                      {[
                        '¿Cuáles son las 3 principales amenazas ahora mismo?',
                        '¿Hay patrones sísmicos que se correlacionen con conflictos?',
                        'Evalúa los riesgos cibernéticos para infraestructuras críticas',
                      ].map((prompt) => (
                        <button
                          key={prompt}
                          onClick={() => {
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        >
                          {msg.role === 'user' ? 'OPERADOR' : 'ANALISTA POLITEIA'}
                        </span>
                ...
                <div className="flex gap-2 items-end">
                  ...
                  >
                    <textarea
                      ...
                      placeholder="Consulta al analista de inteligencia..."
                      rows={1}
