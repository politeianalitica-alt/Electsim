    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7, duration: 0.6 }} className="glass-panel p-3 pointer-events-auto mt-3 border border-[#00BCD4]/30" style={{ background: 'rgba(0, 188, 212, 0.05)' }}>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">RIESGO DE CADENA DE SUMINISTRO</span>
          {totalRisks > 0 && (
            <span className="gotham-tag gotham-tag--critical" style={{ fontSize: '7px', padding: '1px 4px' }}>{totalRisks} ALERTAS</span>
          )}
        </div>
        <div className="flex items-center gap-2">
            ...
            <div className="space-y-3 mt-2 overflow-y-auto styled-scrollbar pr-1 flex-1 pb-4">
              ...
              {marketAlerts.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-[9px] font-mono text-[#FF9500] tracking-widest font-bold">ALERTAS DE IMPACTO EN MERCADOS</span>
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[9px] font-mono text-[var(--text-muted)] tracking-widest">PROVEEDORES CRÍTICOS</span>
                </div>
                {criticalSuppliers.length === 0 ? (
                  <div className="text-[9px] font-mono text-[#00E676] px-2">✓ Todos los nodos Tier 1/2 monitorizados operativos.</div>
                ) : (
                  <div className="space-y-1">
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[9px] font-mono text-[var(--text-muted)] tracking-widest">NODOS CONGESTIONADOS</span>
                </div>
                {(congestedPorts.length === 0 && riskyChokes.length === 0) ? (
                  <div className="text-[9px] font-mono text-[#00E676] px-2">✓ Flujo marítimo global óptimo.</div>
                ) : (
                  <div className="space-y-1">
                    ...
                    {congestedPorts.map((p: any, i: number) => (
                      <div key={`p-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: p.congestion === 'SEVERE' ? '#FF1744' : '#FF9500' }}>
                        ...
                        <div className="flex justify-between items-center text-[8px] font-mono text-[#aaa]">
                          <span>PERMANENCIA: <span className="text-[#fff]">{p.dwell_time}</span></span>
