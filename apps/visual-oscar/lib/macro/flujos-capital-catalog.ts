...
export const FLUJOS_CAPITAL_INDICATORS: PulsoIndicatorMeta[] = [
  ...
  // ─── Cuenta financiera total %PIB ────────────────────────────────────
  {
    ...
    frequency: "quarterly",
    description:
      "Saldo cuenta financiera BoP en millones EUR. Flujos netos entrada/salida (IED+cartera+otros). Positivo=España exporta capital · negativo=importa capital.",
    endpoint: "/api/eurostat/dataset?code=bop_c6_q&filters=geo=ES;partner=WRL_REST;sector10=S1;sectpart=S1;bop_item=FA;stk_flow=NET;currency=MIO_EUR",
    parser: "eurostat-simple",
    accent: "#0EA5E9",
    methodologyNote:
      "FA = Financial Account. Sprint W.3.1d: filtros expandidos (Eurostat requiere partner+stk_flow+currency explícitos). partner=WRL_REST (rest of world), stk_flow=NET, currency=MIO_EUR. Unidad cambiada de %PIB a millones EUR — Eurostat no expone PC_GDP para BoP trimestral. Por identidad contable, FA debe ser ~espejo de CA + KA. Lectura ES post-2013: superávit CC → exporta capital neto.",
    releaseSchedule: "Trimestral · T+90 días",
    confidenceLevel: "high",
