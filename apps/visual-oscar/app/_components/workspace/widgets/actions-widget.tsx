    ...
    >
      {pending.length === 0 ? (
        <div className="flex h-full flex-col items-center justify-center text-center gap-2">
          ...
          <p className="text-sm font-semibold text-[#1d1d1f]">Día limpio</p>
          <p className="text-[11px] text-[#6e6e73]">Sin acciones pendientes</p>
        </div>
      ) : (
            ...
            return (
              <li
                ...
                className={`rounded-lg p-2.5 transition-colors ${
                  done ? "bg-[#fbfbfd] opacity-50" : "bg-[#fbfbfd] hover:bg-[#f5f5f7]/60"
                }`}
              >
                <div className="flex items-start gap-2">
                  <button
                    onClick={() => setCompleted(s => new Set(s).add(a.id))}
                    className="mt-0.5 h-3.5 w-3.5 flex-none rounded border border-[#d2d2d7] hover:border-emerald-500 flex items-center justify-center"
                  >
                    {done && <span className="text-[9px] font-bold text-emerald-400">x</span>}
                  </button>
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-medium leading-snug ${done ? "line-through text-[#6e6e73]" : "text-[#1d1d1f]"}`}>
                      {a.title}
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      <BadgePriority value={a.priority} />
                      <span className={`text-[10px] ${overdue ? "text-red-400 font-semibold" : "text-[#6e6e73]"}`}>
