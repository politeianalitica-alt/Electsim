...
export function AgendaWidget({ events, workspaceId }: AgendaWidgetProps) {
  ...
  return (
    ...
    >
      {sorted.length === 0 ? (
        <p className="text-sm text-slate-400">Sin eventos en las próximas 24 horas.</p>
      ) : (
        <ul className="space-y-2">
          {sorted.slice(0, 6).map(ev => {
            ...
            return (
              <li
                key={ev.id}
                className="flex items-center gap-3 rounded-lg bg-slate-950 p-2.5 hover:bg-slate-800/60 transition-colors"
              >
                <div className="flex flex-col items-center w-12 flex-none">
                  <span className={`text-xs font-bold ${isPast ? "text-slate-600" : "text-slate-200"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-medium leading-snug ${isPast ? "text-slate-500 line-through" : "text-slate-100"}`}>
                    {ev.title}
                  </p>
                  <span
