...
function buildProfile(p: Perfil) {
  // 1. Intención de voto: media ponderada de los 7 ejes (la ideología pesa el doble)
  ...
  for (const { eje, valor, peso } of ejes) {
    const data = VOTO[eje][valor] || {}
    for (const part of PARTIES) voto[part] += (data[part] || 0) * (peso / total)
  }
  // Renormalizar a 100%
  const sum = Object.values(voto).reduce((s, v) => s + v, 0)
  for (const part of PARTIES) voto[part] = Math.round((voto[part] / sum) * 100 * 10) / 10
  // Aproximación final: redondear a entero y reajustar para sumar 100
  const votoInt: Record<Partido, number> = { PP:0, PSOE:0, VOX:0, Sumar:0, Junts:0, PNV:0, Otros:0 }
  let acc = 0
  for (const part of PARTIES) { votoInt[part] = Math.round(voto[part]); acc += votoInt[part] }
  ...
  for (const { eje, valor, peso } of ejes) {
    const data = TEMAS_PESO[eje][valor] || {}
  ...
  // 6. Tamaño estimado del segmento (en miles, deterministic mock)
  const code = `${p.edad}|${p.genero}|${p.estudios}|${p.habitat}|${p.ideologia}|${p.empleo}|${p.religion}`
  let h = 0
  for (let i = 0; i < code.length; i++) h = ((h << 5) - h) + code.charCodeAt(i)
  const segmento = Math.abs(h % 850) + 80  // 80-930K
  ...
  return { voto: votoInt, topTemas, participacion, medios, arquetipo, segmento, posIdeo, ganador }
}
...
export default function MicrodatosPage() {
  ...
  const PRESETS: { id: string; label: string; perfil: Perfil }[] = [
    { id:'joven-urbano-univ',  label:'Joven urbano universitario', perfil:{ edad:'25–34', genero:'Mujer', estudios:'Universitarios', habitat:'Gran ciudad', ideologia:'Centro-izq.', empleo:'Asalariado', religion:'Indiferente' } },
    { id:'pensionista-rural',  label:'Pensionista rural',          perfil:{ edad:'65+',   genero:'Hombre',estudios:'Sin estudios',   habitat:'Rural (<10k)', ideologia:'Centro-dcha.', empleo:'Pensionista', religion:'Practicante' } },
    { id:'autonomo-pyme',      label:'Autónomo de pyme',           perfil:{ edad:'45–54', genero:'Hombre',estudios:'FP',             habitat:'Semiurbano',   ideologia:'Centro-dcha.', empleo:'Autónomo',    religion:'No practicante' } },
    { id:'mujer-urbana',       label:'Mujer urbana progresista',   perfil:{ edad:'35–44', genero:'Mujer', estudios:'Universitarios', habitat:'Urbano (>100k)', ideologia:'Izquierda', empleo:'Asalariado',  religion:'Ateo/agnóstico' } },
    { id:'votante-vox',        label:'Votante VOX tipo',           perfil:{ edad:'45–54', genero:'Hombre',estudios:'Secundaria',     habitat:'Semiurbano',   ideologia:'Derecha',      empleo:'Asalariado',  religion:'No practicante' } },
    { id:'jubilado-urbano-pp', label:'Jubilado urbano conservador',perfil:{ edad:'65+',   genero:'Mujer', estudios:'Secundaria',     habitat:'Urbano (>100k)', ideologia:'Centro-dcha.',empleo:'Pensionista', religion:'Practicante' } },
  ]
      ...
      <main style={{ maxWidth:1500, margin:'0 auto', padding:'24px 28px 80px' }}>
        ...
        <section style={{
          ...
          <div>
            ...
            <p style={{ fontSize:13, opacity:0.7, margin:0, lineHeight:1.5 }}>
              Combina edad, género, estudios, hábitat, ideología, situación laboral y religiosidad. El sistema produce intención de voto, temas prioritarios, perfil mediático y participación esperada.
            </p>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
            <HeroKPI label="Ejes" value="7"/>
        ...
        <section style={{
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:14 }}>
            {(Object.keys(VOTO) as (keyof Perfil)[]).map(eje => (
              <div key={eje} style={{ background:'#FAFAFB', border:'1px solid #ECECEF', borderRadius:10, padding:'12px 14px' }}>
                <h4 style={{ margin:'0 0 8px', fontSize:11, fontWeight:800, color:'#3a3a3d', letterSpacing:'0.06em', textTransform:'uppercase' }}>{eje === 'edad' ? 'Edad' : eje === 'genero' ? 'Género' : eje === 'estudios' ? 'Estudios' : eje === 'habitat' ? 'Hábitat' : eje === 'ideologia' ? 'Ideología' : eje === 'empleo' ? 'Situación laboral' : 'Religiosidad'}</h4>
                <div style={{ overflowX:'auto' }}>
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11.5 }}>
                    ...
                    <tbody>
                      {Object.keys(VOTO[eje]).map((seg, i) => (
