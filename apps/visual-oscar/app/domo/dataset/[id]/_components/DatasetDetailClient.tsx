...
export default function DatasetDetailClient({ id }: { id: string }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      ...
      setIsDirty(false)
      if (isNew && saved?.id) window.location.href = `/domo/dataset/${saved.id}`
    },
  })
    ...
    <div className={styles.root}>
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          <Link href="/domo/dataset" className={styles.breadcrumbLink}>← Datasets</Link>
      ...
      {dataset && (
        <div className={styles.metaStrip}>
          ...
          {dataset.sourcePipelineId && (
            <Link href={`/domo/pipeline/${dataset.sourcePipelineId}`} className={styles.pipelineLink}>
              ⟶ Pipeline origen
            </Link>
          )}
        </div>
