...
import { generateId } from '@/lib/estudio/utils'
import { WIDGET_TYPE_META } from '@/lib/estudio/constants'
...
export default function DashboardBuilderClient({ id }: { id: string }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      ...
      setIsDirty(false)
      if (isNew && saved?.id) window.location.href = `/estudio/dashboard/${saved.id}/editar`
    },
  })
