...
export default function CanvasEditorPage({
  ...
  if (!canvas) {
    return (
      <div className="flex h-full flex-col items-center justify-center">
        <p className="text-lg font-semibold text-slate-200 mb-2">Canvas no encontrado</p>
  ...
  return (
    <div className="flex h-full -mx-7 -my-6">
      {/* Layer panel */}
      <aside className="flex w-56 flex-none flex-col border-r border-slate-800 bg-slate-900 p-3 space-y-4">
        <div>
          <Link href={`/workspaces/${params.workspaceId}/canvas`} className="text-xs text-slate-500 hover:text-slate-300">
            ← Canvas
          </Link>
          <h2 className="text-sm font-bold text-slate-100 mt-1">{canvas.title}</h2>
        </div>
        <div>
          <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">Capas</p>
          <div className="space-y-1">
            {Object.entries(OBJECT_TYPE_CONFIG).map(([type, cfg]) => (
              <button
                key={type}
                className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-[11px] text-slate-300 hover:bg-slate-800 transition-colors"
              >
                <span
      ...
      {/* Detail panel */}
      <aside className="flex w-80 flex-none flex-col border-l border-slate-800 bg-slate-900 overflow-y-auto">
              ...
              {canvas.hypotheses.map(h => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed mb-2">{h.statement}</p>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] text-slate-500">{Math.round(h.confidence * 100)}%</span>
                    </div>
                  </div>
                );
              })}
          ...
          {tab === "timeline" && (
            <div className="space-y-2">
              {canvas.timeline.map(t => (
                <div key={t.id} className="rounded-lg bg-slate-950 p-2.5">
                  <p className="text-[10px] text-slate-500">{t.date}</p>
                  <p className="text-xs text-slate-200 mt-0.5">{t.title}</p>
                </div>
              ))}
            </div>
          )}
