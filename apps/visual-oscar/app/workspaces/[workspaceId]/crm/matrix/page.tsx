  ...
  return (
    <div>
      <div className="mb-4">
        ...
        <h1 className="text-lg font-bold text-slate-100 mt-1">Matriz de posiciones</h1>
      ...
      <div className="overflow-auto rounded-xl border border-slate-800 bg-slate-900">
        <table className="min-w-full border-collapse text-xs">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-slate-900 px-3 py-2 text-left text-slate-400 border-b border-r border-slate-800 min-w-[200px]">
                Actor
              </th>
              {issues.map(issue => (
                <th
                  key={issue.id}
                  className="px-2 py-2 text-center text-slate-400 border-b border-slate-800 min-w-[120px] max-w-[140px]"
                >
          ...
          <tbody>
            {actors.map(actor => {
              ...
              return (
                <tr key={actor.id} className="hover:bg-slate-900/50 transition-colors">
                  <td className="sticky left-0 z-10 bg-slate-950 border-b border-r border-slate-800 px-3 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="font-medium text-slate-200">{actor.displayName}</span>
                  ...
                  {issues.map(issue => {
                    ...
                    return (
                      <td
                        key={issue.id}
                        className="border-b border-slate-800 px-2 py-2 text-center cursor-pointer hover:bg-slate-800/60 transition-colors"
