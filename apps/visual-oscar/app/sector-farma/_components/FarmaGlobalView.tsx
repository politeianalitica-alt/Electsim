'use client'
/**
 * <FarmaGlobalView /> · vista Visión Global del shell Farma v3.
 *
 * Contiene el dashboard original `/sector-farma`: KPIs + desabastecimientos
 * + ranking de laboratorios + distribución ATC + buscador + empresas +
 * reguladores + licitaciones + áreas + intel panel + cuaderno.
 *
 * Diferencia con la página plana anterior:
 *   - Catálogos (EMPRESAS / REGULADORES / ÁREAS) se leen ahora desde
 *     `lib/farma/catalogos/*.json` con fuente verificable.
 *   - No incluye AppHeader / outer div: los provee FarmaShell.
 *
 * Datos en vivo desde CIMA AEMPS · sin auth · refresh 30 min.
 */
import { useEffect, useMemo, useState } from 'react'
import Link from 'next/link'
import { EMPRESAS_FARMA, REGULADORES_FARMA, AREAS_TERAPEUTICAS } from '@/lib/farma/catalogos'
...
export function FarmaGlobalView() {
...
function EmpresasGrid() {
 ...
 <li key={e.ticker}>
 <a href={e.web || e.cnmv_url} target="_blank" rel="noreferrer" style={{
