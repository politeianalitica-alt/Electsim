...
export default function Ibex35DetailPage() {
  ...
  const dossier = useMemo(
    () => IBEX35_FIXTURE.find(d => d.slug === slug),
    [slug],
  )
    ...
    return (
      <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
        ...
        <main style={{ maxWidth: 900, margin: '40px auto', padding: '0 28px' }}>
          <Link href="/ibex35" style={{ color: '#0071e3', textDecoration: 'none', fontSize: 13 }}>⟵ Volver al listado IBEX 35</Link>
  ...
  const sector = dossier.tags.find(t => t.toLowerCase().startsWith('sector:'))
  const sectorLabel = sector ? sector.split(':')[1].replace(/-/g, ' ') : null
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'var(--font-body)' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 28px 64px' }}>
        <Link href="/ibex35" style={{ color: '#0071e3', textDecoration: 'none', fontSize: 13, display: 'inline-block', marginBottom: 18 }}>
          ⟵ Volver a IBEX 35
        </Link>

        {/* Hero */}
        <header style={{
          background: 'linear-gradient(135deg, #1F4E8C 0%, #2D4A8A 100%)',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 12 }}>
              {sectorLabel && (
                <span style={chipDarkStyle()}>{sectorLabel}</span>
              )}
