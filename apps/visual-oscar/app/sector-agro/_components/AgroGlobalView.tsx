'use client'
/**
 * <AgroGlobalView /> · vista Visión Global del shell Agro v3.
 *
 * PRESERVA el dashboard original con datos en vivo de World Bank
 * (8 series de indicadores agrícolas) + catálogos curados (empresas,
 * reguladores, áreas, programas). Los catálogos ahora vienen de
 * `lib/agro/catalogos/*.json` con fuente verificable.
 *
 * El shell se encarga de AppHeader + auth + outer div · esta vista
 * sólo aporta el contenido.
 */
...
export function AgroGlobalView() {
  ...
  return (
    <div>
        ...
        {/* ROW 4: Programas */}
        <Panel title="Programas y políticas activas" subtitle="PAC · PERTE · Plan Sequía · Ley Cadena" marginBottom>
          <ProgramasGrid
            programas={PROGRAMAS_AGRO.map((p) => ({
              programa: p.programa,
              estado: p.estado,
              presupuesto_b: p.presupuesto_eur != null ? p.presupuesto_eur / 1_000_000_000 : 0,
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:14, marginBottom:14 }}>
          ...
          <Panel title="Reguladores y operadores" subtitle="Marco institucional agro nacional + UE">
            <RegLista
              reguladores={REGULADORES_AGRO.map((r) => ({
                nombre: r.siglas,
                full: r.nombre,
                web: r.web,
