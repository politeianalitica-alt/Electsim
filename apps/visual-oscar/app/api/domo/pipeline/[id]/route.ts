...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const res = await fetch(`${BACKEND}/api/domo/pipeline/${params.id}`, { next: { revalidate: 0 } })
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const res  = await fetch(`${BACKEND}/api/domo/pipeline/${params.id}`, {
      method: 'PUT',
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const res = await fetch(`${BACKEND}/api/domo/pipeline/${params.id}`, { method: 'DELETE' })
