/**
 * Catch-all proxy · /api/brain/* → FastAPI /api/v1/brain/*
 *
 * Propaga X-User-Id. Timeout 60s (las llamadas LLM pueden tardar 20-40s
 * en cold start).
 */
...
async function passthrough(
  ...
  const target = `/api/v1/brain${tail ? '/' + tail : ''}${url.search}`
