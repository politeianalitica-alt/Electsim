...
export interface CountryInflation { countries: string[]; days: number; series: Record<string, Array<{ date: string; metric: string; value: number }>> }
...
export async function GET(req: NextRequest) {
  ...
  const r = await callBackend<CountryInflation>(
    `/api/macro-finance/hicp?countries=${encodeURIComponent(countries)}&days=${encodeURIComponent(days)}`,
    { cache: 'no-store' },
  )
  if (r.data) {
