/**
 * GET /api/entsoe/embalses?zone=ES&days=56 · Sprint Energía S3+
 *
 * Embalses y almacenamiento hidráulico (documentType A72 · processType A16,
 * resolución semanal P7D) de una zona de oferta UE, vía ENTSO-E Transparency
 * Platform. Ventana `days` por defecto 56 (8 semanas); acotada al rango 7..365.
 ...
 * con mensaje claro. Caché HTTP s-maxage 1h (datos semanales).
 */
import { NextRequest, NextResponse } from 'next/server'
import { periodForDays } from '@/lib/entsoe/client'
import { fetchHydroReservoirs } from '@/lib/entsoe/extended'
...
export async function GET(req: NextRequest) {
  ...
  const days = clamp(Number(req.nextUrl.searchParams.get('days') || 56), 7, 365)
  const { periodStart, periodEnd } = periodForDays(days)

  const r = await fetchHydroReservoirs(zone, periodStart, periodEnd)
